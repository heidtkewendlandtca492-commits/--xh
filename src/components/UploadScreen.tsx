...
export function UploadScreen({ onUpload }: { onUpload: (uploads: { data: File | string, type: AssetType }[]) => void }) {
  ...
  const UploadBox = ({ type, title }: { type: AssetType, title: string }) => (
    <div className="border-2 border-dashed border-neutral-300 rounded-xl p-6 flex flex-col items-center justify-center relative hover:bg-neutral-50 transition-colors h-48">
      {files[type] ? (
